...
class User {
  ...
  async get(identifier) {
    try {
      ...
      const [results] = await this.slaveDB.execute(
        `SELECT userId, vin, fullname, email, created_at, updated_at FROM User WHERE ${queryField}=?`,
        [identifier],
      );
      return results?.[0];
    } catch (error) {
  ...
  async create(email, fullname, password) {
    try {
      ...
      const [results] = await this.masterDB.execute(
        "INSERT INTO User (vin, fullname, email, password, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())",
        [vin, fullname, email, await encrypt(password)],
      );
  ...
  async verify(identifier, password) {
    try {
      ...
      const [results] = await this.slaveDB.execute(`SELECT * FROM User WHERE ${queryField}=? AND password=?`, [
        identifier,
  ...
  async update(userId, fullname, email) {
    try {
      const [results] = await this.masterDB.execute("UPDATE User SET fullname=?, email=?, updated_at=NOW() WHERE userId=?", [
        fullname,
        email,
        userId,
      ]);
